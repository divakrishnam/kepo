...
class Kepo(object):
    ...
    def generateURL(self, NPM, PASSWORD, PROYEK, NILAI, KET, Pembimbing, userrepo, namarepo, pertemuan):
        lengkap = self.sudahLengkap(
            NPM, PASSWORD, PROYEK, NILAI, KET, Pembimbing, userrepo, namarepo, pertemuan)
    ...
    def loginSiap(self, NPM, password):
        ...
        while success == False:
            try:
                siapnpm = self.driver.find_elements_by_class_name(
                    'cap-main')[7].text
                if siapnpm == NPM:
                    status = True
                success = True
    ...
    def getNilaiSemester(self, semester):
        ...
        while success == False:
            try:
                ...
                self.driver.get(
                    "http://siap.poltekpos.ac.id/siap/modul/simpati/index.php?mnux=nilai.mahasiswa&mdlid=43")
                self.driver.find_element_by_xpath(opsisemester).click()
                self.driver.find_element_by_xpath(
                    "//input[@value='Cari' and @name='Cari']").click()
                tabel = self.driver.find_elements_by_xpath(
